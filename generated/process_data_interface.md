...
This document outlines the Process Data Interface (PDI) for the Licensing Self-Certification Portal (LSCP) of the Buildings Department (BD). It details how the system handles data processing internally and integrates with external systems.
...
2.  **System Data Process Interface:** Defines the approach for handling data processes within the LSCP system.
3.  **External Interfaces:** Defines the approach for integrating the LSCP with external systems, including interface specifications.
...
This section describes the physical data design and components of the physical process within the LSCP. The interface makes the database, implemented using the physical environment, appear as the required system logical data model to the processing components. This aims to ease system implementation and future maintenance.

For every incoming process request, the function accepts and handles the input first. Then, the function updates and queries the database.

The following diagram illustrates the position of the Process Data Interface (PDI) relative to the universal function model:

**In/Out data process flow diagram**

[Diagram: In/Out data process flow diagram] (See `pdi_t1.md` for the image)

## 3. External Interfaces

This section defines the approach for system integration with external systems, including interface specifications.
...
*   **Description:** The LSCP system will call stored procedures in the SMIS database to import necessary data. The exact data fields to be imported will need to be defined in the detailed design phase.
...
*   **Access from Buildings Departments (BD) Departmental Portal:** The link to access the LSCP will be: `https://lscp.bd.gov.hk`
