...
This document outlines the Process Data Interface (PDI) for the Licensing Self-Certification Portal (LSCP) of the Buildings Department (BD). It covers the data processing within the system and its integration with external systems.

## 1. Introduction

The PDI document is structured into three main sections:
...
3.  **External Interfaces:** Defines the handling approach for system integration with external systems, including detailed interface specifications.
...
This section describes the physical data design and components of the physical process. The interface ensures that the database, implemented using the physical environment, appears as the Required System Logical Data Model to the processing components. This approach aims to simplify system implementation and future maintenance.

For every incoming process request, the function handles the input first, then updates and queries the database.

**In/Out Data Process Flow Diagram:**

![In/Out Data Process Flow Diagram](image2)

This diagram illustrates the position of the PDI relative to the universal function model.

## 3. External Interfaces

This section details the integration of the LSCP with various external systems.
...
*   **Data Exchange:** Data will be transferred directly between the databases via stored procedures.
...
*   **Error Handling:** Stored procedures should include error handling to manage potential issues during data transfer and logging.
...
*   **In/Out Data Process Flow Diagram**
    ...
    ![In/Out Data Process Flow Diagram](image4)
...
*   **Description:** The system can use `https://dp2.bd.hksarg/bravo/intranetSignOn` to redirect to BRAVO. Also, the LSCP system could be redirected to BRAVO through a URL call with specified parameters.
